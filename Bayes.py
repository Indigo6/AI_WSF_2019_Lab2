...
class Gaussian_NB:
    ...
    def fit(self,x,y):
        ...
        self.labels = list(set(y))
        self.label_num = len(self.labels)
        self.mus = np.zeros((self.label_num,self.dim),dtype=float)
        self.sigmas = np.zeros((self.label_num,self.dim),dtype=float)
        self.num_per_label = np.zeros((self.label_num),dtype=int)
        self.priori = np.zeros((self.label_num),dtype=float)
    ...
    def update(self,x,y):
        ...
        #update priori
        self.priori = self.num_per_label/self.sample_num

        #update sigmas
        for i in range(self.sample_num):
            label = self.label_index[y[i]]
            temp = (x[i]-self.mus[label])**2
            self.sigmas[label] += temp
        for i in range(self.label_num):
            self.sigmas[i] /= self.num_per_label[i]
        self.sigmas = np.sqrt(self.sigmas)    
    ...
    #获取概率 P(x | label), , 默认使用 log 
    #此处label已映射
    def Gaussian(self,label,x):
        mu = self.mus[label]
        sigma = self.sigmas[label]

        #cond_probs[i]: 第i维的对数类条件概率 log(P(x_i | label))
        #通过训练出来的 mu 和sigma 高斯分布而得
        cond_probs = np.ones((self.dim),dtype=float)
        cond_probs /= (sigma * np.sqrt(2*np.pi))
        cond_probs = np.log(cond_probs)
        temp =  -((x-mu)**2 /2 /(sigma**2))
        cond_probs += temp
        predict_prob = np.log(self.priori[label]) + cond_probs.sum() 
        return predict_prob
